...
public final class SQLiteDatabase extends SQLiteClosable {
    ...
    private void beginTransaction(SQLiteTransactionListener transactionListener,
        ...
        acquireReference();
        try {
            B_beginTransaction(transactionListener, exclusive);
        } finally {
            releaseReference();
        }
    }
    ...
    private boolean yieldIfContendedHelper(boolean throwIfUnsafe, long sleepAfterYieldDelay) {
        acquireReference();
        try {
            return B_yieldIfContendedHelper(throwIfUnsafe, sleepAfterYieldDelay);
        } finally {
            releaseReference();
        }
    }
    ...
    public void validateSql(@NonNull String sql, @Nullable CancellationSignal cancellationSignal) {
        B_validateSql(sql, cancellationSignal);
    }
    ...
    private void B_initDriver() {
        try {
            ...
            throw new IllegalStateException("Database driver " + DRIVER_CLASS + " not found!");
        }
    }
    ...
    public void B_validateSql(String sql, CancellationSignal cancellationSignal) {
        try {
            ...
            throw new SQLiteException("Invalid SQL statement!", e);
        } finally {
    ...
    private synchronized boolean B_yieldIfContendedHelper(boolean throwIfUnsafe, long sleepAfterYieldDelay) {
        ...
        try {
            ...
            throw new SQLiteException("Failed to commit current transaction!", e);
    ...
    private boolean B_inTransaction() {
        try {
            ...
            throw new SQLiteException("Failed to get transaction status!", e);
        }
    }
    ...
    private synchronized void B_endTransaction() {
        ...
        Transaction thisTransaction = transactionStack.pop();
        if(!thisTransaction.successful)
            transactionStackInvalidated = true;

        if(transactionStack.isEmpty()) {
            try {
                if (!transactionStackInvalidated) {
                    //If all transaction ended successfully, commit
                    thisTransaction.listener.onCommit();
                    connection.commit();
                } else {
                    //If a transaction failed, rollback
                    thisTransaction.listener.onRollback();
                    connection.rollback();
                    transactionStackInvalidated = false;
                }
                connection.setAutoCommit(true);
            } catch(java.sql.SQLException e) {
    ...
    private static class NoOpSQLiteTransactionListener implements SQLiteTransactionListener {
        private static NoOpSQLiteTransactionListener INSTANCE = new NoOpSQLiteTransactionListener();
