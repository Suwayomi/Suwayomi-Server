...
import suwayomi.tachidesk.manga.impl.track.tracker.yamtrack.dto.YTMediaItem
import suwayomi.tachidesk.manga.impl.track.tracker.yamtrack.dto.YTSearchResponse
import suwayomi.tachidesk.manga.impl.track.tracker.yamtrack.dto.toTrackSearch
...
class YamtrackApi(
    ...
    private fun mediaPath(
        source: String,
        mediaId: String,
    ): String = "/media/manga/${encodeSegment(source)}/${encodeSegment(mediaId)}/"
    ...
    suspend fun getMediaItem(
        ...
        withIOContext {
            try {
                with(json) {
                    authClient
                        .newCall(GET(apiUrl(mediaPath(source, mediaId))))
                        .awaitSuccess()
                        .parseAs<YTMediaItem>()
                }
            } catch (_: Exception) {
    ...
    suspend fun updateMedia(
        ...
        val request =
            Request
                ...
                .patch(body.toString().toRequestBody(jsonMime))
                .build()
    ...
    suspend fun deleteMedia(track: Track) =
        withIOContext {
            val (source, mediaId) = Yamtrack.parseTrackingUrl(track.tracking_url) ?: return@withIOContext
            authClient
                .newCall(DELETE(url = apiUrl(mediaPath(source, mediaId))))
                .awaitSuccess()
                .close()
        }
