...
object ProtoBackupImport : ProtoBackupBase() {
    ...
    private fun restoreManga(
        ...
        val chapters = if (flags.includeChapters) backupManga.chapters else emptyList()
        val categories = if (flags.includeCategories) backupManga.categories else emptyList()
        val history = if (flags.includeHistory) backupManga.history else emptyList()
        val tracking = if (flags.includeTracking) backupManga.tracking else emptyList()
